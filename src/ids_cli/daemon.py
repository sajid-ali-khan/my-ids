...
class DaemonManager:
    ...
    def start(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            command = self._resolve_run_server_command(python_exe)
            if not command:
                return (
                    False,
                    "Unable to locate run_server module or script. Ensure it is installed or run from the project root.",
                )

            # Start server using module or script
            ...
            ConfigManager.append_log(
                f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Server started (PID: {proc.pid})"
            )
            ...
            ConfigManager.append_log(
                f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Error starting server: {e}"
            )
    ...
    def stop(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            ConfigManager.clear_pid()
            ConfigManager.append_log(
                f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Server stopped (PID was: {pid})"
            )
            ...
            ConfigManager.append_log(
                f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Error stopping server: {e}"
            )
    ...
    @staticmethod
    def _resolve_run_server_command(python_exe: str) -> Optional[list]:
        """Resolve the command to run the server module or script."""
        if importlib.util.find_spec("run_server") is not None:
            return [python_exe, "-m", "run_server"]
        if importlib.util.find_spec("scripts.run_server") is not None:
            return [python_exe, "-m", "scripts.run_server"]

        candidate = Path(__file__).resolve().parents[2] / "scripts" / "run_server.py"
        if candidate.exists():
